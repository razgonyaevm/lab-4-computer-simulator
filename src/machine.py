...
class ControlUnit:
    ...
    def _execute_mov(self, mode: int, reg_d: int, reg_s: int, pc: int) -> None:
        ...
        Args:
            mode: Числовой код режима адресации (AddressingMode), декодированный
                из второго байта заголовка инструкции. Определяет способ выборки операндов.
            reg_d: Индекс целевого регистра (Register), декодированный из третьего байта
                заголовка инструкции. Используется как приемник результата или база адреса.
            reg_s: Индекс регистра-источника (Register), декодированный из четвертого байта
                заголовка инструкции. Используется как источник данных или база адреса.
            pc: Текущее значение счетчика команд (Program Counter), указывающее на байтовый
                адрес этой инструкции в памяти команд.
        """
