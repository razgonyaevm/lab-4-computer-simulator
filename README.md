...
* **Особенности реализации:**
    ...
    * **Эффективность векторизации:** Сложение 4-х элементов с помощью векторных команд занимает всего 23 такта, в
      то время как эквивалентный скалярный цикл на Lisp требует 416 тактов (ускорение в ~18 раз). Это
...
| ИТОГО:                                                                       | 104 тактов                                                                                                            | 416 тактов                               | 23 такта                                                                |
...
* **Память команд (Instruction Memory):** Объём - 4096 ячеек. Адресуется в байтах. Содержит скомпилированные
  ...
  основной программы.
* **Память данных (Data Memory):** Объём - 4096 байт (1024 машинных слова). Адресуется в байтах, при этом одно машинное
  слово занимает 4 последовательных байта в ОЗУ. Разделена на три области:
    1. *Секция статических строк:* Начинается с адреса `0` и заполняется транслируемыми строковыми литералами.
       Каждому символу (слову) выделяется 4 байта
    2. *Секция глобальных переменных:* Заполняется транслятором последовательно вслед за строковыми литералами (каждой
       переменной выделяется 4 байта).
    3. *Кадр аппаратного стека (Call Stack):* Расположен в конце памяти и растет вниз в направлении к адресу `0`.
       Вершина стека изначально установлена на адрес `4092`. Смещение для локальных переменных относительно `FP` 
       рассчитывается по байтовой формуле `offset = 8 + arg_idx * 4` (где 8 байт резервируются под сохраненные 
       4-байтовые `FP` и `SP` на стеке).
* **Регистровый файл:**
...
* **Длина вычислений:** 3_897_096 тактов.
* **Входной поток:** отсутствует.
