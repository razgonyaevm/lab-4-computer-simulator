...
| 0030 : "!" (cstr)                        | ; Статическая строка в ОЗУ
|  ... : ...                               |
| 0100 : Dynamic Input Buffer ("Maxim")    | ; Динамический буфер ввода
|  ... : ...                               |
| 4084 : Stack: [arg1 (n)]                 | ; Локальный аргумент функции
| 4088 : Stack: [Saved FP (4092)]          | ; Сохраненный указатель кадра
| 4092 : Stack: [Return PC (204)]          | ; Сохраненный адрес возврата
+------------------------------------------+
```
...
|:-----------|:------------|:-----------------|:------|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **MOV**    | `0x01`      | `IMMEDIATE`      | 3     | Запись 32-битной константы Payload в `Rd`: <br> $\text{Rd} \leftarrow \text{Payload}$                                                                                                        |
|            |             | `REGISTER`       | 2     | Копирование значения из регистра `Rs` в `Rd`: <br> $\text{Rd} \leftarrow \text{Rs}$                                                                                                          |
| **LOAD**   | `0x02`      | `DIRECT`         | 3     | Прямое чтение слова из ОЗУ по адресу `[Payload]` в регистр `Rd`: <br> $\text{Rd} \leftarrow \text{M}[\text{Payload}]$                                                                        |
|            |             | `INDIRECT`       | 3     | Косвенное чтение из стека по адресу `[Rs + offset]` в регистр `Rd`: <br> $\text{Rd} \leftarrow \text{M}[\text{Rs} + \text{offset}]$                                                          |
| **STORE**  | `0x03`      | `DIRECT`         | 3     | Прямая запись регистра `Rs` в ячейку ОЗУ по адресу `[Payload]`: <br> $\text{M}[\text{Payload}] \leftarrow \text{Rs}$                                                                         |
|            |             | `INDIRECT`       | 3     | Косвенная запись регистра `Rs` в стек по адресу `[Rd + offset]`: <br> $\text{M}[\text{Rd} + \text{offset}] \leftarrow \text{Rs}$                                                             |
| **PUSH**   | `0x04`      | `REGISTER`       | 2     | Запись `Rs` на стек: `SP = SP - 4`, $\text{M}[\text{SP}] \leftarrow \text{Rs}$                                                                                                               |
| **POP**    | `0x05`      | `REGISTER`       | 2     | Извлечение со стека в `Rd`: $\text{Rd} \leftarrow \text{M}[\text{SP}]$, `SP = SP + 4`                                                                                                        |
| **ADD**    | `0x10`      | `IMMEDIATE`      | 3     | Сложение `Rd` с константой: $\text{Rd} \leftarrow (\text{Rd} + \text{Payload}) \pmod{2^{32}}$                                                                                                |
|            |             | `REGISTER`       | 2     | Регистровое сложение: $\text{Rd} \leftarrow (\text{Rd} + \text{Rs}) \pmod{2^{32}}$                                                                                                           |
| **SUB**    | `0x11`      | `REGISTER`       | 2     | Регистровое вычитание: $\text{Rd} \leftarrow (\text{Rs} - \text{Rd}) \pmod{2^{32}}$                                                                                                          |
| **MUL**    | `0x12`      | `REGISTER`       | 2     | Регистровое умножение: $\text{Rd} \leftarrow (\text{Rs} \times \text{Rd}) \pmod{2^{32}}$                                                                                                     |
| **DIV**    | `0x13`      | `REGISTER`       | 2     | Целочисленное регистровое деление: $\text{Rd} \leftarrow \lfloor \text{Rs} / \text{Rd} \rfloor$                                                                                              |
...
| **VLOAD**  | `0x20`      | `DIRECT`         | 6     | Загрузка вектора (4 слова) в регистр `V_reg` из ОЗУ: <br> $\text{V}_\text{reg[i]} \leftarrow \text{M}[\text{Payload + i}]$                                                                   |
| **VSTORE** | `0x21`      | `DIRECT`         | 6     | Сохранение `V_reg` (4 слова) в ячейки ОЗУ: <br> $\text{M}[\text{Payload + i}] \leftarrow \text{V}_\text{reg[i]}$                                                                             |
| **VADD**   | `0x22`      | `REGISTER`       | 5     | Поэлементное векторное сложение векторов: <br> $\text{V_dest[i]} \leftarrow (\text{V}_\text{dest[i]} + \text{V}_\text{src[i]}) \pmod{2^{32}}$                                                |
| **VSUB**   | `0x23`      | `REGISTER`       | 5     | Поэлементное векторное вычитание векторов: <br> $\text{V}_\text{dest[i]} \leftarrow (\text{V}_\text{dest[i]} - \text{V}_\text{src[i]}) \pmod{2^{32}}$                                        |
| **VMUL**   | `0x24`      | `REGISTER`       | 5     | Поэлементное векторное умножение векторов: <br> $\text{V}_\text{dest[i]} \leftarrow (\text{V}_\text{dest[i]} \times \text{V}_\text{src[i]}) \pmod{2^{32}}$                                   |
| **VDIV**   | `0x25`      | `REGISTER`       | 5     | Поэлементное векторное целочисленное деление векторов                                                                                                                                        |
| **VCMP**   | `0x26`      | `REGISTER`       | 5     | Поэлементное векторное сравнение (записывает 1, если элементы равны, иначе 0): <br> $\text{V}_\text{dest[i]} \leftarrow (\text{V}_\text{dest[i]} == \text{V}_\text{src[i]} \ ? \ 1 \ : \ 0)$ |
| **JMP**    | `0x30`      | `IMMEDIATE`      | 3     | Безусловный переход по адресу Payload: $\text{PC} \leftarrow \text{Payload}$                                                                                                                 |
...
| **CALL**   | `0x33`      | `IMMEDIATE`      | 3     | Вызов функции: сохранение PC на стек и переход на Payload: <br> $\text{M[SP-4]} \leftarrow \text{PC}+8$, `SP = SP - 4`, $\text{PC} \leftarrow \text{Payload}$                                |
...
| **IRET**   | `0x41`      | `REGISTER`       | 3     | Выход из обработчика прерывания: восстановление флагов `SR` и счетчика команд `PC` со стека                                                                                                  |
